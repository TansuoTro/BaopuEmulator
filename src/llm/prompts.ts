...
export function buildRecommendPrompt(
  ...
  return JSON.stringify({
    ...
    instruction: '综合所有数据（画像、高考、毕业意向、情景回答、开放回答），严格遵循SYSTEM_PROMPT中的"毕业意向×专业"推荐规则。输出：(1)personality_sketch—3~5句人物侧写；(2)top_majors—Top5按score降序，每专业给match_reason(结合高考数据和毕业意向)、future_path(根据毕业意向+高考分数+省排名+选科，分析该专业的未来发展：考公路径/升学路径/就业路径/灵活就业路径，指出最推荐的路径并说明原因，例如"物理学考公岗位很少，建议优先考虑升学→科研或转量化金融"、"自动化分数偏低则优先推荐直接就业→智能制造工程师"、"会计学考公有大量岗位(税务局/审计署)，建议大三开始备考")、risk、suitable_for、not_suitable_for；(3)final_note—一句话，必须呼应毕业意向和高考数据。output_format:{"type":"recommend","personality_sketch":"...","summary":{"confidence":...},"top_majors":[{"major_name":"...","score":...","match_reason":[...],"future_path":"...","risk":[...],"suitable_for":[...],"not_suitable_for":[...]}],"final_note":"..."}',
  });
}
