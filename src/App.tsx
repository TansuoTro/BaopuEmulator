...
import { MajorNode, DynamicQuestion, UserProfile, RecommendationResult as RR } from './types';
import UniverseScene from './components/3d/UniverseScene';
import '@fortawesome/fontawesome-free/css/all.min.css';

/* ── API call helper (direct to DeepSeek for now, Netlify proxy in production) ── */
...
const Bar: React.FC<{label:string;val:number;icon:string}> = ({label,val,icon}) => {
  ...
  return <div className="flex items-center gap-2 text-xs"><span className="w-14 text-right text-white/50 flex items-center justify-end gap-1"><i className={`fas ${icon} w-3`}></i>{label}</span><div className="flex-1 h-2 bg-white/10 rounded-full"><div className={`h-full rounded-full ${c}`} style={{width:`${val}%`}}/></div><span className="w-7 text-right tabular-nums text-white/40">{val}</span></div>;
};
...
const App: React.FC = () => {
  ...
  const handleGaokaoSubmit=useCallback((info:typeof store.gaokaoInfo)=>{store.setGaokaoInfo(info);},[store]);
  ...
  useEffect(()=>{
    ...
    ds(store.apiKey,buildDynamicPrompt(profile,detectConflicts(profile),hist as unknown as string[]))
      .then(raw=>{
        ...
        else if(Array.isArray(d))qs=d.filter(x=>x&&'stem' in x) as unknown as DynamicQuestion[];
        if(qs.length===0)qs=FIXED_QUESTIONS.slice(0,5).map((f,i)=>({id:`D${i+1}`,question_type:'choice',target_discrimination:[f.primary_dim],stem:`再确认：${f.stem}`,options:f.options.map(o=>({key:o.key,text:o.text})),input_hint:'',expected_signal:`确认${f.primary_dim}`}));
        store.setDynamicQuestions(qs.slice(0,5));
      }).catch(e=>{console.warn('Dynamic failed:',e);store.setDynamicQuestions(FIXED_QUESTIONS.slice(0,5).map((f,i)=>({id:`D${i+1}`,question_type:'choice',target_discrimination:[f.primary_dim],stem:`再确认：${f.stem}`,options:f.options.map(o=>({key:o.key,text:o.text})),input_hint:'',expected_signal:`确认${f.primary_dim}`})));})
  ...
  const handleOpenAnswer=useCallback(async(a:string)=>{
    ...
    try{
      ...
      store.applyOpenResult(r as {tags?:string[]});
      store.answerOpen(a);
  ...
  useEffect(()=>{
    ...
    ds(store.apiKey,prompt).then(raw=>{
      const d=raw as RR;
      store.finalizeRecommendation(d);
    }).catch(e=>{console.warn('Recommend failed, using code-only:',e);store.finalizeRecommendation({final_note:'基于代码侧匹配结果（LLM推荐生成失败，请检查API Key）'});})
    .finally(()=>{setLoading(false);loadingRef.current=false;});
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDark?'bg-[#080816] text-white':'bg-zinc-50 text-zinc-900'}`}>
      ...
      {phase==='idle'&&(
        <div className="max-w-md mx-auto px-4 pt-16 sm:pt-20 pb-10 text-center">
          ...
          <div className="mt-8 space-y-4 text-left">
            ...
            <input type="password" value={apiInput} onChange={e=>setApiInput(e.target.value)} placeholder="sk-..." className={`w-full px-4 py-3 rounded-lg font-mono text-sm ${isDark?'bg-white/5 border-white/10 text-white placeholder-white/20':'bg-zinc-100 border-zinc-300 text-zinc-900'}`}/>
            <button onClick={handleStart} disabled={!apiInput.trim()} className="w-full py-3.5 rounded-lg font-bold bg-indigo-500 hover:bg-indigo-600 text-white disabled:opacity-30">🌌 进入专业宇宙</button>
          </div>
        </div>
      )}
      ...
      {phase==='gaokao'&&(
        <div className="max-w-xl mx-auto p-4 sm:p-6 space-y-5 overflow-y-auto" style={{height:'calc(100vh-56px)'}}>
          <h2 className="text-xl font-bold text-center">高考信息</h2>
          {(()=>{
            const [g,setG]=useState(store.gaokaoInfo);
            ...
            return <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div><label className="text-xs text-white/50 block mb-1">年份</label><select value={g.year} onChange={e=>u('year',+e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm">{Array.from({length:26},(_,i)=>2025-i).map(y=><option key={y} value={y}>{y}</option>)}</select></div>
                <div><label className="text-xs text-white/50 block mb-1">省份</label><select value={g.province} onChange={e=>u('province',e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"><option value="">选</option>{['北京','上海','广东','江苏','浙江','山东','河南','湖北','四川','湖南','福建','安徽','河北','辽宁','陕西','重庆','江西','天津','云南','广西','山西','贵州','黑龙江','吉林','甘肃','内蒙古','新疆','海南','宁夏','青海','西藏'].map(p=><option key={p}>{p}</option>)}</select></div>
                <div><label className="text-xs text-white/50 block mb-1">类型</label><select value={g.gaokao_type} onChange={e=>u('gaokao_type',e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"><option value="新高考">新高考</option><option value="旧高考-理综">旧高考-理综</option><option value="旧高考-文综">旧高考-文综</option></select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-white/50 block mb-1">总分</label><input type="number" value={g.total_score||''} onChange={e=>u('total_score',+e.target.value)} placeholder="0~750" className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"/></div>
                <div><label className="text-xs text-white/50 block mb-1">排名(可选)</label><input type="number" value={g.provincial_rank||''} onChange={e=>u('provincial_rank',+e.target.value)} placeholder="选填" className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"/></div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div><label className="text-xs text-white/50 block mb-1">语文</label><input type="number" value={g.chinese||''} onChange={e=>u('chinese',+e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"/></div>
                <div><label className="text-xs text-white/50 block mb-1">数学</label><input type="number" value={g.math||''} onChange={e=>u('math',+e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"/></div>
                <div><label className="text-xs text-white/50 block mb-1">英语</label><input type="number" value={g.english||''} onChange={e=>u('english',+e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-sm"/></div>
              </div>
              <button onClick={()=>handleGaokaoSubmit(g)} disabled={!can} className="w-full py-3.5 rounded-lg font-bold bg-indigo-500 hover:bg-indigo-600 text-white disabled:opacity-30">开始测评</button>
            </div>;
          })()}
        </div>
      )}
        ...
        <div className={`assessment-shell ${isDark?'':'assessment-shell-light'}`}>
          <div className="hidden lg:block left-col">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4">
              <h3 className="text-xs font-bold text-white/60 mb-3">{phase==='fixed'?'阶段一：基础画像':phase==='dynamic'?'阶段二：深度消歧':'阶段三：动机补足'}</h3>
          ...
          <div className="center-col">
            <div className="lg:hidden bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-3 text-xs text-white/40">{phase==='fixed'?'基础画像':phase==='dynamic'?'深度消歧':'动机补足'} {curQ+1}/{totalQ}</div>
            ...
            <div className="question-area">
              ...
              {phase==='dynamic'&&curDynamic&&(
                <div className="bg-white/5 backdrop-blur-md border border-purple-500/20 rounded-xl p-4 space-y-4">
              ...
              {phase==='open'&&OPEN_QUESTIONS[store.openIndex]&&(
                <div className="bg-white/5 backdrop-blur-md border border-emerald-500/20 rounded-xl p-4 space-y-4">
                  ...
                  <textarea rows={3} id="open-input" placeholder={OPEN_QUESTIONS[store.openIndex].input_hint||'自由回答...'} className="w-full p-3 rounded bg-white/5 border border-white/10 text-white text-sm resize-none"/>
                  <button onClick={()=>{const el=document.getElementById('open-input') as HTMLTextAreaElement;if(el?.value.trim())handleOpenAnswer(el.value.trim());}} disabled={loading} className="w-full py-3 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm disabled:opacity-30">提交</button>
                </div>
              )}
            </div>
          </div>
      ...
      {phase==='recommend'&&store.recommendation&&(
        <div className="overflow-y-auto" style={{height:'calc(100vh-56px)'}}>
          <div className="max-w-5xl mx-auto p-3 sm:p-4 pb-20 space-y-4">
            <div className="bg-gradient-to-r from-indigo-600/20 via-purple-600/20 to-indigo-600/20 rounded-xl p-5 text-center"><h2 className="text-xl font-bold">测评完成</h2><div className="mt-2"><span className={`inline-block px-4 py-2 rounded-full text-sm font-bold ${store.recommendation.confidence_breakdown.total>=70?'bg-emerald-500/20 text-emerald-300':'bg-amber-500/20 text-amber-300'}`}>置信度 {store.recommendation.confidence_breakdown.total}%</span></div></div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-xl p-4"><h3 className="text-sm font-bold text-white/80 mb-3">能力画像</h3><ProfileBars p={profile}/></div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 overflow-y-auto max-h-80">
                <h3 className="text-sm font-bold text-white/80 mb-3">Top 5 推荐专业</h3>
                <div className="space-y-3">
                  {store.recommendation.top_majors.map((m,i)=><div key={m.major.id} onClick={()=>setSelectedMajor(m.major)} className={`p-3 rounded-lg border cursor-pointer transition-all ${selectedMajor?.id===m.major.id?'border-indigo-400':'border-white/10 hover:border-white/30'}`}>
                    <div className="flex justify-between items-center"><span className="text-white font-semibold text-sm">{i+1}. {m.major.name}</span><span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300">{m.final_score}分</span></div>
                    <div className="h-1 bg-white/10 rounded-full mt-2"><div className="h-full bg-emerald-400 rounded-full" style={{width:`${m.final_score}%`}}/></div>
                    <div className="mt-2 text-xs text-white/40">{m.major.tags.join(' · ')}</div>
                  </div>)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex gap-3"><i className="fas fa-comment-dots text-indigo-400"/> <p className="text-white/60 text-sm">{store.recommendation.final_note||'综合能力画像和专业匹配度，以上推荐供参考。建议结合个人兴趣和高考实际情况做最终决策。'}</p></div>
            <button onClick={store.reset} className="w-full py-3 rounded-lg border border-white/20 text-white/60 hover:bg-white/10 transition-all text-sm">重新测评</button>
          </div>
        </div>
      )}
